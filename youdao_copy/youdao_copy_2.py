...
def handler(signum, frame):
    ...
    while True:
        ...
        if in_put == 'e':
            ...
            ins_copy.debug = not ins_copy.debug
            break
        elif in_put == 'm':
            action = raw_input("please input start|stop|restart to start|stop|restart clicktocopy.py\n:")
            if action == 'start':
                ins_copy.start_mouce_mode()
            if action == 'stop':
                ins_copy.stop_mouce_mode()
            if action == 'restart':
                ins_copy.restart_mouce_mode()
            break
        elif in_put == 's':
...
class Youdao_copy(object):  
    ...
    def restart_mouce_mode(self):
        self.stop_mouce_mode()
        self.start_mouce_mode()
        ...
        while(self.run_flag):
            try :
                try :
                    ...
                    word = "你好"
                if word_new != word:
                    self.debug_print("paste : " + word_new)
                    #如果不存在非字母字符
