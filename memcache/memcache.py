...
"""
StringIO :This module implements a file-like class, StringIO, that reads and writes a string buffer
可以像文件一样读写缓存
import StringIO
output = StringIO.StringIO() #实例化一个对象（分配一个缓存）
In [20]: output.
output.buf         output.closed      output.isatty      output.pos         output.readlines   output.tell        output.writelines
output.buflist     output.flush       output.len         output.read        output.seek        output.truncate
output.close       output.getvalue    output.next        output.readline    output.softspace   output.write
"""
try:
...
A class that represents thread-local data. Thread-local data are data whose values
are thread specific. To manage thread-local data, just create an instance of local
(or a subclass) and store attributes on it:

mydata = threading.local()
mydata.x = 1
threading.local 类的属性是线程本地的，子类也是
...
class Client(local):
    ...
    def __init__(self, servers, debug=0, pickleProtocol=0,
        ...
        #实例化StringIO
        file = StringIO()
        # pickler 把对象序列化写入文件中，StringIO对象具有文件属性
        try:
    ...
    # ['127.0.0.1:11211','191.168.45.74:11211']
    ...
    def set(self, key, val, time=0, min_compress_len=0):
        '''Unconditionally 无条件地 sets a key to a given value in the memcache.
...
#server="unix:port"
#server="inet:ipv4:port"
#server="ipv4:port"
#server="inet6:ipv6:port"
class _Host(object):
