...
USAGE = """%s [options]
eg: python rados_multi_client_local.py -p rep-pool -i volume-1 -b 4M -f 1 -c 40
"""
...
def main():
    ...
    for i in range(opts.client):
        ...
        cmd1="python /root/rados_client.py -p %s -i %s -b %s -f %dG -o %dG" % (opts.pool, opts.image, opts.block, opts.filesize, opts.offset + i*opts.filesize)
        cmd.append(cmd1)
    process = []
