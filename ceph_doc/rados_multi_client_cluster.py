...
USAGE = """%s [options]
eg: python rados_multi_client_cluster.py -p rep-pool -i volume-1 -b 4M -f 1 -c 80
...
def main():
    ...
    for i in range(len(cliet_ip)):
        cmd1="ssh %s python /root/rados_multi_client_local.py -p %s -i %s -b %s -f %d -c %d -o %d" % (cliet_ip[i], opts.pool, opts.image, opts.block, opts.filesize/opts.client, host_client_num, i*host_client_num*opts.filesize/opts.client)
        cmd.append(cmd1)
    process = []
