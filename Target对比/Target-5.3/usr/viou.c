...
int receive_msg_unix(int fd, struct Message *msg) {
    ...
    if (n != sizeof(msg->DataLength)) {
        Log_error("fail to read datalength\n");
		return -EINVAL;
    }
    ...
    if (n != sizeof(msg->Type)) {
        Log_error("fail to read type\n");
		return -EINVAL;
    }

    n = readn_unix(fd, &msg->Offset, sizeof(msg->Offset));
    if (n != sizeof(msg->Offset)) {
        Log_error("fail to read offset\n");
		return -EINVAL;
    }
    
	Log_debug("receive_msg type:%u offset:%ld length:%lu\n", msg->Type, (long int)msg->Offset, msg->DataLength);
	...
	if (msg->DataLength > 0) {
		...
		if (n != msg->DataLength) {
            Log_error("Cannot read full from fd, %lu vs %d\n", msg->DataLength, n);
			free(msg->Data);
			return -EINVAL;
	    }
	}
...
void* response_process_unix(void *arg) {
    ...
    while (1) {
        ...
        case TypeOK:
            break;
        case TypeError:
            Log_error("Receive error for response %d of seq %lu: %s\n",
                                        resp->Type, (unsigned long)resp->Seq, (char *)resp->Data);
            /* fall through so we can response to caller */
            break;
        case TypeEOF:
            Log_error("Receive eof for response %d of seq %lu: %s\n",
                                        resp->Type, (unsigned long)resp->Seq, (char *)resp->Data);
            break;
        case TypeTimeout:
            Log_error("Receive timeout for response %d of seq %lu: %s\n",
                                        resp->Type, (unsigned long)resp->Seq, (char *)resp->Data);
            break;
        case TypeClose:
        ...
        if (req == NULL) {
            Log_warning("Unknow response sequence %lu\n", (unsigned long)resp->Seq);
            free(resp->Data);
            continue;
        }
...
int process_request_unix(struct viou_connection *conn, void *buf, uint64_t count, off_t offset,
                uint32_t type) {
    ...
    if (conn->state != CLIENT_CONN_STATE_OPEN) {
        Log_error("Cannot queue in more request, Connection is closed");
        rc = -EFAULT;
	    pthread_mutex_unlock(&conn->mutex);
        goto free;
    }
    ...
    pthread_mutex_lock(&req->mutex);
    Log_debug("seq:%ld type:%u offset:%ld length:%lu\n",req->Seq, req->Type, (long)req->Offset, req->DataLength);
    rc = send_request_unix(conn, req);
    if (rc < 0) {
        Log_error("type:%u send_request return:%d\n", req->Type, rc);
        goto out;
    }

    pthread_cond_wait(&req->cond, &req->mutex);
    Log_debug("command return seq:%ld type:%u offset:%ld length:%lu\n",req->Seq, req->Type, (long)req->Offset, req->DataLength);

    if (req->Type != TypeOK) {
        Log_error("type:%u offset:%ld length:%lu\n", req->Type, (long)req->Offset, req->DataLength);
        rc = -EFAULT;
    }
...
int read_at_unix(struct viou_connection *conn, void *buf, uint64_t count, off_t offset) {
    return process_request_unix(conn, buf, count, offset, TypeRead);
}

int write_at_unix(struct viou_connection *conn, void *buf, uint64_t count, off_t offset) {
    return process_request_unix(conn, buf, count, offset, TypeWrite);
}

int xcopy_at_unix(struct viou_connection *conn, struct xcopy *xcopy) {
    ...
    ret = process_request_unix(conn, buf, sizeof(struct xcopy), 0, TypeXCopy);
    free(buf);
	return ret;
}

int unmap_at_unix(struct viou_connection *conn, uint64_t count, off_t offset) {
	return process_request_unix(conn, NULL, count, offset, TypeUnmap);
}

int caw_at_unix(struct viou_connection *conn, void *buf, uint64_t count, off_t offset) {
    return process_request_unix(conn, buf, count, offset, TypeCAW);
}

int write_same_at_unix(struct viou_connection *conn, void *buf, size_t block_size, uint64_t count, off_t offset) {
    int ret;
    char *p = malloc(8+block_size);
	*(uint64_t *)p = htole64(count);
	memcpy(p+8, buf, block_size);
	Log_debug("write same length:   %lu\n", *(uint64_t *)&p[0]);
    ret = process_request_unix(conn, p, (uint64_t)(8+block_size), offset, TypeWriteSame);
    free(p);
	return ret;
}
...
struct viou_connection *new_viou_connection(char *socket_path) {
    ...
    if (rc < 0) {
        Log_error("fail to init conn->mutex");
        exit(-EFAULT);
    }
...
int shutdown_viou_connection1(struct viou_connection *conn) {
	...
	HASH_ITER(hh, conn->msg_table, req, tmp) {
        ...
        pthread_mutex_lock(&req->mutex);
        req->Type = TypeError;
        Log_error("Cancel request %lu due to disconnection", (unsigned long)req->Seq);
        pthread_mutex_unlock(&req->mutex);
        pthread_cond_signal(&req->cond);
	}
	pthread_mutex_unlock(&conn->mutex);
