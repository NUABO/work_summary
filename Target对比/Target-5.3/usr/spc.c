...
int spc_inquiry(int host_no, struct scsi_cmd *cmd)
{
	...
	if (!evpd) { /* no VPD, return standard Inquiry data */
		...
		data[2] = 0x05;	/* SPC-3 */
		data[3] = 0x12;
		data[5] = 0x08; /* A Third-Party Copy (3PC) Enable the XCOPY */
		data[7] = 0x02; /* CmdQueue */
...
int spc_report_target_groups(int host_no, struct scsi_cmd *cmd)
{
	int ret;
	unsigned char key = ILLEGAL_REQUEST;
	uint16_t asc = ASC_INVALID_OP_CODE;
	
	Log_debug("spc_report_target_groups cmd:0x%x buffer length: %u\n", cmd->scb[0], scsi_get_in_length(cmd));
...
sense:
	Log_error("spc_report_target_groups sense, cmd:0x%x key: %d, asc: %d\n", cmd->scb[0], (int)key, (int)asc);
	cmd->offset = 0;
	scsi_set_in_resid_by_actual(cmd, 0);
