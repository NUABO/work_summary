...
static int sbc_receive_copy_result(int host_no, struct scsi_cmd *cmd) {
	int ret;
	unsigned char key = ILLEGAL_REQUEST;
	uint16_t asc = ASC_INVALID_OP_CODE;

	Log_debug("xcopy sbc_receive_copy_result entry, cmd:0x%x buffer length: %u\n", cmd->scb[0], scsi_get_in_length(cmd));
...
sense:
	Log_error("xcopy sbc_receive_copy_result sense, cmd:0x%x key: %d, asc: %d\n", cmd->scb[0], (int)key, (int)asc);
	cmd->offset = 0;
	scsi_set_in_resid_by_actual(cmd, 0);
	scsi_set_out_resid_by_actual(cmd, 0);
...
static int sbc_xcopy(int host_no, struct scsi_cmd *cmd) {
	int ret;
	unsigned char key = ILLEGAL_REQUEST;
	uint16_t asc = ASC_INVALID_OP_CODE;

    Log_debug("xcopy sbc_xcopy entry, cmd:0x%x buffer length: %u\n", cmd->scb[0], scsi_get_in_length(cmd));
...
sense:
	Log_error("xcopy sbc_xcopy sense, cmd:0x%x key: %d, asc: %d\n", cmd->scb[0], (int)key, (int)asc);
	cmd->offset = 0;
	scsi_set_in_resid_by_actual(cmd, 0);
	scsi_set_out_resid_by_actual(cmd, 0);
...
static int sbc_compare_and_write(int host_no, struct scsi_cmd *cmd) {
    int ret;
	unsigned char key = ILLEGAL_REQUEST;
	uint16_t asc = ASC_INVALID_OP_CODE;

	Log_debug("COMPARE_AND_WRITE sbc_compare_and_write, cmd:0x%x buffer length: %u\n", cmd->scb[0], scsi_get_in_length(cmd));
...
sense:
	Log_error("COMPARE_AND_WRITE sbc_compare_and_write sense, cmd:0x%x key: %d, asc: %d\n", cmd->scb[0], (int)key, (int)asc);
	cmd->offset = 0;
	scsi_set_in_resid_by_actual(cmd, 0);
	scsi_set_out_resid_by_actual(cmd, 0);
...
static tgtadm_err sbc_lu_init(struct scsi_lu *lu)
{
	...
	strncpy(lu->attrs.product_id, "VeSpace-Disk", sizeof(lu->attrs.product_id));
...
static struct device_type_template sbc_template = {
	...
	.ops		= {
		...
		{spc_illegal_op,},
		{spc_illegal_op,},
		{sbc_xcopy,},        /* EXTENDED_COPY 0x83 */
		{sbc_receive_copy_result,}, /* RECEIVE_COPY_RESULTS 0x84 */
		{spc_illegal_op,},
		{spc_illegal_op,},
		{spc_illegal_op,},
