...
struct ringbuffer *init(char *shm_file)
{
    ...
    if (fd < 0) {
        Log_error("open file: %s return %d err:%s\n", ringbuf->shm_file, fd, strerror(errno));
        goto error;
    }
    ...
    if (ringbuf->buffer == (void *)-1) {
        Log_error("mmap size:%u return -1\n", (unsigned int)(ringbuf->size));
        goto error;
    }
    Log_debug("mmap size:%u fd:%d\n", (unsigned int)(ringbuf->size), fd);
    ...
    if (rc < 0) {
        Log_error("pthread_mutex_init return -1\n");
        goto error;
    }
...
struct cmnd * add(struct ringbuffer *rbuf, void *buf, uint64_t length, int64_t offset,
                uint32_t type, uint64_t seq, uint32_t *idx)
{
    ...
    do {
		//Unmap有可能length很大，但其数据是空的，不需要占用huge命令字段
        if (length <= CMD_COMMON_LEN || type == TypeUnmap) {
		...
		Log_debug("add cmd idx:%u, state:%u, length:%lu, seq:%llu, type:%u, offset:%lld\n", *idx,
						cmd->state, cmd->length, (long long unsigned)cmd->seq, cmd->type, (long long int)cmd->offset);

		if (cmd->type == TypeWrite || cmd->type == TypeXCopy || cmd->type == TypeWriteSame || cmd->type == TypeCAW) {
