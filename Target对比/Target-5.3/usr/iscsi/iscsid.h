...
struct iscsi_session {
	...
	/* linked to sessions_list 全局变量 session.c:33 */
	...
	/* links pending tasks (task->c_list) 是按照task->req.statsn小到大排的*/
	struct list_head pending_cmd_list;

	uint32_t exp_cmd_sn;  //ExpCmdSN :the next expected command by the target
...
struct iscsi_connection {
	...
	uint32_t stat_sn; //the status sequence number on this connection
	uint32_t exp_stat_sn; //ExpStatSN is used by the initiator to acknowledge status

	uint32_t cmd_sn; // Command Sequence Number
	uint32_t exp_cmd_sn; // the next expected command sequence number at the target
	uint32_t max_cmd_sn; // the maximum CmdSN acceptable at the target from this initiator
    ...
    /* links tasks iscsi_task->c_list  */
	struct list_head tx_clist;

    /* links tasks (iscsi_task->c_siblings) */
...
struct iscsi_target {
    ...
    /* links all sessions (iscsi_session->slist) */
...
struct iscsi_portal {
    /* linked to iscsi_portals_list 全局变量 iscsid.c:47 */
