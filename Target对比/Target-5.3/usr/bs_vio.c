...
static void bs_vio_request(struct scsi_cmd *cmd)
{
    ...
    case WRITE_12:
    case WRITE_16:
		Log_debug("cmd->scb[0]: %x(WRITE_x) length:%u, offset:%lu\n", cmd->scb[0], cmd->tl, cmd->offset);
	...
	case READ_12:
	case READ_16:
		Log_debug("cmd->scb[0]: %x(READ_x) length:%u, offset:%lu\n", cmd->scb[0], cmd->tl, cmd->offset);
	...
	case SYNCHRONIZE_CACHE:
        Log_debug("cmd->scb[0]: %x(SYNCHRONIZE_CACHE) length:%u, offset:%lu\n", cmd->scb[0], cmd->tl, cmd->offset);
	    break;
    case WRITE_SAME:
	case WRITE_SAME_16:
		...
		if (cmd->scb[1] & 0x08) {
			...
			ret = unmap_at(vio->conn, cmd->tl, cmd->offset);
		    if (ret) {
                result = SAM_STAT_CHECK_CONDITION;
				key = HARDWARE_ERROR;
		...
		ret = write_same_at(vio->conn, buf, block_size, cmd->tl, offset);
		if (ret) {
	...
	case UNMAP:
        ...
        for (i = 0; i < umap_state->count; i++) {
            ret = unmap_at(vio->conn, ((uint64_t)(umap_state->nlbas[i]) << 9), (umap_state->lba[i] << 9));
			if (ret) {
                result = SAM_STAT_CHECK_CONDITION;
				key = HARDWARE_ERROR;
				asc = ASC_INTERNAL_TGT_FAILURE;
	...
	case COMPARE_AND_WRITE:
		...
		//ret = caw_at(vio->conn, scsi_get_out_buffer(cmd), scsi_get_out_length(cmd), cmd->offset);
		ret = caw_at(vio->conn, scsi_get_out_buffer(cmd), scsi_get_out_length(cmd), block_size*get_lba_internal(cmd->scb));
		if (ret) {
            result = SAM_STAT_CHECK_CONDITION;
		    key = MISCOMPARE;
	...
	case MAINT_PROTOCOL_IN:
        if ((cmd->scb[1] & 0x1f) == MPI_REPORT_TARGET_PGS) {
			...
			result = process_request(vio->conn, NULL, 0, 0, TypeRtpg);
        }
	    break;
	default:
